...
function parseMusicConfiguration(source: string): MusicConfiguration {
    ...
    const requestedPreset = (values.get("preset") || values.get("layout") || "free").toLowerCase()
    const normalizedPreset = requestedPreset === "intimed" ? "untimed" : requestedPreset
    ...
    return {
        preset: (["untimed", "timed", "free"].includes(normalizedPreset)
            ? normalizedPreset
...
function musicScoreAssetUrl(config: MusicConfiguration): string {
    // Quartz provides the GitHub Pages repository subpath on every generated page.
    const basePath = document.body.dataset.basepath || ""
    const url = new URL(`${basePath}/static/music-score-app/index.html`, window.location.origin)
...
function buildMusicScoreFrame(source: string): HTMLElement {
    ...
    frame.addEventListener("load", () => {
        frame.contentWindow?.postMessage(
            {type: "music-score-configuration", config: {...config, theme: currentMusicScoreTheme()}},
...
function setupMusicScoreApps() {
    document.querySelectorAll<HTMLPreElement>('pre[data-language="music-score"]').forEach((pre) => {
        if (pre.dataset.musicScoreMounted === "true") return

        pre.dataset.musicScoreMounted = "true"
        pre.replaceWith(buildMusicScoreFrame(pre.textContent || ""))
    })
...
window.addEventListener("message", (event) => {
    if (event.origin === window.location.origin && event.data?.type === "resize") {
