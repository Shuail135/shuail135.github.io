...
const BEAT_WIDTH = 54
const STAFF_BOTTOM_Y = 102
const STAFF_HALF_STEP = 6
...
const DEFAULT_PIANO_SAMPLE_URL = "https://tonejs.github.io/audio/salamander/"
...
const PIANO_SAMPLE_MIDI = [...PIANO_SAMPLES.keys()]
const SCALE_MODES = {
...
const KEY_OPTIONS = [
    [0, "C"], [1, "C♯ / D♭"], [2, "D"], [3, "E♭"], [4, "E"], [5, "F"],
    [6, "F♯ / G♭"], [7, "G"], [8, "A♭"], [9, "A"], [10, "B♭"], [11, "B"],
]
const MAJOR_KEY_SIGNATURES = new Map([
    [0, 0], [1, -5], [2, 2], [3, -3], [4, 4], [5, -1],
    [6, 6], [7, 1], [8, -4], [9, 3], [10, -2], [11, 5],
])
const TIME_SIGNATURES = ["2/4", "3/4", "4/4", "5/4", "6/8", "7/8", "9/8", "12/8"]
const SHARP_DIATONIC = {
...
const sharedPianoSamplers = new Map()

class AcousticPianoSampler {
    ...
    nearestSampleMidi(midi) {
        return PIANO_SAMPLE_MIDI.reduce((nearest, candidate) =>
    ...
    createVoice(midi, startTime, duration, volume) {
        ...
        gain.gain.exponentialRampToValueAtTime(level, startTime + 0.008)
...
function parseTonic(value) {
    ...
    return pitchClasses[normalized] ?? clamp(Number(normalized) || 0, 0, 11)
}
...
function noteTokenToMidi(value) {
    ...
    return clamp((Number(match[3]) + 1) * 12 + pitchClass, 0, 127)
}
...
function readConfiguration(element) {
    ...
    const bpm = clamp(Number(element.getAttribute("bpm")) || 120, 20, 300)
    ...
    const octave = clamp(Number(element.getAttribute("octave")) || 4, 1, 7)
...
class MusicScoreApp extends HTMLElement {
    constructor() {
        super()
    ...
    async connectedCallback() {
        ...
        this.bindEvents()
        this.renderAll()
        this.setStatus(`Loading acoustic piano · ${this.engine.kind}`)
        this.preloadCurrentOctave().then((loaded) => {
            if (this.isConnected) {
                this.setStatus(loaded
                    ? `Ready · ${this.engine.kind} · Acoustic piano`
                    : `Ready · ${this.engine.kind} · Synth fallback`)
            }
        })
    }
      ...
      <div class="app" tabindex="0" aria-label="Interactive music score and piano">
        <div class="toolbar">
          <label id="mode-control">Recording mode
            <select id="mode">
              <option value="untimed">No timing · quarter notes</option>
        ...
        <div class="footer">
          ...
          <span>Keys: A W S E D F T G Y H U J K · Salamander Grand Piano, CC BY 3.0</span>
        </div>
      </div>
    `
    ...
    bindEvents() {
        ...
        this.ui.bpm.addEventListener("change", () => {
            this.config.bpm = clamp(Number(this.ui.bpm.value) || 120, 20, 300)
            this.ui.bpm.value = String(this.config.bpm)
            this.engine.setBpm(this.config.bpm)
        })
    ...
    changeOctave(delta) {
        this.config.octave = clamp(this.config.octave + delta, 1, 7)
        this.renderKeyboard()
        this.updateControlState()
    ...
    createFallbackVoice(context, midi, startTime, duration = null, volume = 0.7) {
        ...
        if (duration != null) {
            const releaseAt = startTime + Math.max(0.04, duration)
            gain.gain.setValueAtTime(Math.max(0.0001, volume * 0.17), releaseAt)
            gain.gain.exponentialRampToValueAtTime(0.0001, releaseAt + 0.16)
    ...
    async play() {
        ...
        notes.forEach((note, index) => {
            ...
            const timer = window.setTimeout(() => {
                ...
                window.setTimeout(() => token === this.playbackToken && this.markPlaying(index, false), duration * 1000)
            }, Math.max(0, (noteStart - context.currentTime) * 1000))
        ...
        this.playbackTimers.push(window.setTimeout(() => {
            ...
            this.renderStaff()
        }, (0.1 + finalBeat * secondsPerBeat + 0.25) * 1000))
    ...
    handleStaffPointer(event) {
        ...
        if (event.target.closest?.(".msa-note")) {
            const midi = Number(event.target.closest(".msa-note").dataset.midi)
            if (midi >= 0) {
                this.ensureAudio().then(async (context) => {
                    await this.preparePianoSamples([midi])
                    this.createVoice(context, midi, context.currentTime, 0.3, 0.65)
                })
            }
            this.setStatus(noteName(midi))
            return
        }
        ...
        const midi = this.staffInput === "rest"
            ? -1
            : clamp(diatonicToMidi(diatonic) + this.accidental, 0, 127)
    ...
    renderStaff() {
        ...
        clef.textContent = this.config.clef === "treble" ? "𝄞" : "𝄢"
        svg.append(clef)
        ...
        if (this.config.mode === "timed") {
            ...
            for (let beat = 1; beat <= endBeat; beat += 1) {
                ...
                svg.append(svgElement("line", {
                    x1: x, y1: 54, x2: x, y2: 102, class: "beat-guide",
                }))
            }
            ...
            for (let beat = grid; beat <= endBeat; beat += grid) {
                if (Math.abs(beat - Math.round(beat)) < 0.0001) continue
                const x = musicStart + beat * BEAT_WIDTH
    ...
    drawNote(svg, note, index) {
        ...
        const head = svgElement("ellipse", {
            cx: x, cy: y, rx: 7.2, ry: 4.8,
            transform: `rotate(-18 ${x} ${y})`,
            class: `note-head${hollow ? " hollow" : ""}`,
        })
        group.append(head)

        const signatureDegrees = keySignature > 0
            ? [3, 0, 4, 1, 5, 2, 6].slice(0, keySignature)
            : [6, 2, 5, 1, 4, 0, 3].slice(0, Math.abs(keySignature))
        ...
        if (!whole) {
            ...
            for (let flag = 0; flag < flags; flag += 1) {
                ...
                const path = svgElement("path", {
                    d: stemsUp
                        ? `M ${stemX} ${startY} Q ${stemX + 13} ${startY + 5} ${stemX + 8} ${startY + 15}`
                        : `M ${stemX} ${startY} Q ${stemX - 13} ${startY - 5} ${stemX - 8} ${startY - 15}`,
                    class: "flag",
                    fill: "none",
                    "stroke-width": 2,
    ...
    drawRest(group, x, duration) {
        ...
        if (duration >= 0.75) {
            group.append(svgElement("path", {
                ...
                class: "rest-symbol", fill: "none", "stroke-width": 3,
            }))
            return
        }
